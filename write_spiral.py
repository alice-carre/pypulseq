...
# Ici test
# adc_dwell1 = (np.pi*k_radius**2/adc_samples_desired)*system.grad_raster_time
...
% block otherwise there will be a gap in between

gz_spoil.delay = pp.calc_duration(gx)
gx_spoil.delay = gz_spoil.delay
gy_spoil.delay = gz_spoil.delay
gx_combined = pp.add_gradients((gx, gx_spoil), system=system)
gy_combined = pp.add_gradients((gy, gy_spoil), system=system)
gz_combined = pp.add_gradients((gz_reph, gz_spoil), system=system)
"""
...
for s in range(0, N_slices):
    ...
    # seq.add_block(gx_combined, gy_combined, gz_combined, adc)
