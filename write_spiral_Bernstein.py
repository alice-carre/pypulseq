...
for index in range(N_shot):
    ...
    for c in range(k_samples):
        ...
        ka[0][c] = np.real(lbd * teta * np.exp(1j * (teta + 2 * np.pi * index / N_shot)))
        ka[1][c] = np.imag(lbd * teta * np.exp(1j * (teta + 2 * np.pi * index / N_shot)))
    # plt.plot(ka[0], ka[1])
...
for index in range(0, N_shot):
    List_spiral_grad_shape.append(List_grad_and_slew_rate_original[index][0])
    spiral_grad_shape = List_spiral_grad_shape[index]
